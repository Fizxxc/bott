import config from '../resources/Utils/Config.js'
...
if (/mongo/.test(config.Settings.database)) {
  Database = class MongoDB {
    constructor(url) {
      ...
      this.connection = this.url || config.Settings.database
      this.model = {
        database: {},
      }
      this.data = {}
...
} else if (/json/.test(config.Settings.database)) {
  Database = class Database {
    data = {}
    file = path.join(process.cwd(), config.Settings.database)
