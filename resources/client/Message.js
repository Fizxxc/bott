...
import { delay, jidNormalizedUser, getContentType, generateWAMessageFromContent, proto, prepareWAMessageMedia } from "@whiskeysockets/baileys";
import fs from "fs";
import chalk from "chalk";
import util from "util";
import { fileURLToPath } from "url";
import { Config, infoServer, smsg } from "../index.js"
import * as Func from "../libary/Function.js";
...
import quote from "../scapers/qc.js";
import { appenTextMessage } from "../libary/Serialize.js";


const antilink = JSON.parse(fs.readFileSync('./resources/database/group/antilink.json'))
const premium = JSON.parse(fs.readFileSync('./resources/database/roles/premium.json'))
const autoai = JSON.parse(fs.readFileSync('./resources/database/users/autoai.json'))
    ...
    if (db.users[m.sender].afkTime > -1) {
      ...
      user.afkReason = ''
        ...
        switch (isCommand ? m.command.toLowerCase() : false) {
                ...
                if (Config.Settings.typeMenu === 1) {
                    ...
                    contextInfo: {
                        ...
                        forwardedNewsletterMessageInfo: {
                            ...
                            newsletterName: "Runtime " + Func.runtime(process.uptime()),
                        },
                        externalAdReply: {
                            title: `${Config.Information.Bot.name}`,
                            body: "",
                            thumbnail: fs.readFileSync("./resources/assets/img/logo.png"),
                            sourceUrl: "",
                    ...
                    client.sendMessage(m.from, {
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterName: Func.runtime(process.uptime()),
                                newsletterJid: "120363265419579977@newsletter"
                            },
                            externalAdReply: {
                            ...
                            message: {
                                ...
                                interactiveMessage: proto.Message.InteractiveMessage.create({
                                    contextInfo: {
                                        ...
                                        businessMessageForwardInfo: { businessOwnerJid: "6287777056787@s.whatsapp.net" },
                                        externalAdReply: {
                                            title: Config.Settings.Title,
            ...
            case "thanks":
            case " tqto": {
                let tqto = ["Allah SWT.", "Nabi Muhammad SAW.", "My Parent.", "Chocoo (Me).", "Xyz Teamm.", "All my friends."];
                let thanks = `*Thanks To*\n\n`;
            ...
            case "addmenu": {
                ...
                fs.writeFileSync('./resources/Utils/Config.js', `export default ${util.inspect(Config)}`)
            ...
            case "getfeaturs": {
                ...
                let a = "case  " + `'${cases}'` + fs.readFileSync("./resources/client/Message.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                m.reply(a)
            }
                break
            case "addpremium":
            ...
            case "upprem": {
                ...
                premium.push(user)
                fs.writeFileSync('./resources/database/roles/premium.json', JSON.stringify(premium))
            ...
            case "getsesi": {
                ...
                await client.sendMessage(m.from, { document: fs.readFileSync("./resources/Session/creds.json"), fileName: "creds.json", mimeType: "application/json" }, { quoted: m })
            }
                break
            case "join":
            case "joingroup":
            ...
            case "resetsesi": {
                ...
                await fs.readdir("./resources/Session", async function (err, files) {
                    if (err) {
                        console.error(err)
                    ...
                    await file.forEach(function (a) {
                        fs.unlinkSync(`./resources/Session/${a}`)
                    })
                })
            ...
            case "youtubeaudio": {
                ...
                break
            case "tiktok": {
                if (!m.text) return m.reply("Send the url");
                await client.sendList(m.from, '*Tiktok Downloader*', '', { title: 'Click Me :)', sections: [{ title: "Please choose", highlight_label: 'Recomennded', rows: [{ title: "Video Tiktok", description: "Download Video Tiktok", id: '.tiktokdl ' + m.text }, { title: "Audio Tiktok", description: "Download Audio Tiktok", id: '.tiktokaudiodl ' + m.text }] }] })
            }
                break
            case 'tiktokdl':
            case 'ttdl':
            case 'tt': {
                ...
                if (res.status === false) return m.reply("Video Not Found")
            ...
            case 'tiktokmusic': {
                ...
                if (!audio) return m.reply("Video Not Found")
            ...
            case 'ig': {
                if (!m.text) return m.reply("Send the url")
                if (!/instagram.com/.test(m.text)) return m.reply("Invalid URL");
                ...
                for (let i = 0; i < res.length; i++) {
                    if (res[i].includes("https://scontent.cdninstagram.com")) {
                        await client.sendMessage(m.from, { image: { url: res[i] } }, { quoted: m });
                    } else {
                        await client.sendMessage(m.from, { video: { url: res[i] } }, { quoted: m });
                    }
                }
            }
                break
            ...
            case 'stikermeme': {
                try {
                    ...
                    let sticker = await (await import("../libary/Sticker.js")).writeExif(api, { packName: Config.Settings.packName, packPublish: Config.Settings.packPublish })
            ...
            case "s":
                try {
                    if (/image|video|webp/.test(quoted.msg.mimetype)) {
                        ...
                        let sticker = await (await import("../libary/Sticker.js")).writeExif({ mimetype: quoted.msg.mimetype, data: media }, exif)
                        await m.reply({ sticker })
                        ...
                        for (let id of m.mentions) {
                            ...
                            let sticker = await (await import("../libary/Sticker.js")).writeExif(media, { packName: Config.Settings.packName, packPublish: Config.Settings.packPublish })
                            await m.reply({ sticker })
                        }
            ...
            case "tourl": {
                ...
                break
            // Pinterest Command - By Adriansyah
            case 'pin':
            case 'pinterest': {
                if (!m.text) return m.reply("Teks?")
                const [query, countStr] = m.text.split("|");
                const rules = JSON.parse(fs.readFileSync('./resources/database/function/pinterest.json'));
                if (rules.some(rule => m.text.includes(rule))) return m.reply("Sorry, the query is not allowed.")
                let count = countStr ? parseInt(countStr) : 1;
                if (!query) return m.reply("Teks?")
                ...
                try {
                    ...
                    console.error(error);
                    return client.reply(m.from, 'Sorry, this feature error, try again tomorrow!', m);
                }
            }
                break;
            ...
            case 'addrulespin':
            case 'addpinrule': {
                if (!m.text) return m.reply("Type the rules")
                const rules = JSON.parse(fs.readFileSync('./resources/database/function/pinterest.json'))
                if (rules.includes(m.text)) return m.reply("Rules already exist")
                rules.push(m.text)
                fs.writeFileSync('./resources/database/function/pinterest.json', JSON.stringify(rules))
                m.reply("Rules added")
            }
                break
            ...
            case 'delpinrule': {
                if (!m.text) return m.reply("Type the rules")
                const rules = JSON.parse(fs.readFileSync('./resources/database/function/pinterest.json'))
                if (!rules.includes(m.text)) return m.reply("There are no rules")
                rules.splice(rules.indexOf(m.text), 1)
                fs.writeFileSync('./resources/database/function/pinterest.json', JSON.stringify(rules))
                m.reply("Rules removed")
            }
                break //By Adrian Jomok
            case 'get':
            case 'fetch': {
                if (!m.text) return m.reply("Send the url")
            ...
            case "toimage": {
                ...
                let b = "./resources/temp/" + quoted.id + ".png"
            ...
            case "toaudio": {
                ...
                let videoFilePath = `./resources/temp/${quoted.id}.mp4`;
                fs.writeFileSync(videoFilePath, Buffer.from(videoBuffer));
                let audioFilePath = `./resources/temp/${quoted.id}.mp3`;
            ...
            case "ltmp": {
                ...
                let a = fs.readdirSync("./resources/temp");
            ...
            case "deletetmp": {
                ...
                for (let b of a) {
                    if (b !== ".nomedia") {
                        fs.unlinkSync("./resources/temp/" + b);
                    }
                }
            ...
            case 'cekkhodam': {
                ...
                const isModule = (await import("../scapers/khodam.js")).default
            ...
            case "antilink": {
                ...
                if (m.text === "on") {
                    ...
                    fs.writeFileSync("./resources/database/group/antilink.json", JSON.stringify(antilink));
