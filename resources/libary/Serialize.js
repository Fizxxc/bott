...
import { jidNormalizedUser, extractMessageContent, areJidsSameUser, downloadMediaMessage, proto, generateWAMessageFromContent, generateWAMessage, prepareWAMessageMedia } from "@whiskeysockets/baileys";
...
export default async function smsg(client, msg, store) {
    try {
        ...
        m.public = Config.Settings.self
        m.isCreator = m.sender && Config.Owner.includes(m.sender.replace(/\D+/g, ""));
        ...
        if (m.message) {
            ...
            if (m.msg?.contextInfo?.quotedMessage) {
                ...
                if (m.quoted.message) {
                    ...
                    m.quoted.isCreator = m.quoted.sender && Config.Owner.includes(m.quoted.sender.replace(/\D+/g, ""));
                }
            }
        }
        ...
        m.reply = async (text, options = {}) => {
            if (typeof text === "string") {
                return await client.sendMessage(m.from, { text, ...options, contextInfo: { forwardedNewsletterMessageInfo: { newsletterName: "Powered By Chocopyy", newsletterJid: "120363265419579977@newsletter" } } }, { quoted: m, ephemeralExpiration: m.expiration, ...options })
